...
export async function action({ request }) {
  ...
  if (action === "mark_embed_enabled") {
    await prisma.countryBlockerSettings.upsert({
      ...
      create: { shop, appEmbedEnabled: true }
    });
  ...
  if (action === "mark_embed_disabled") {
    await prisma.countryBlockerSettings.upsert({
      ...
      create: { shop, appEmbedEnabled: false }
    });
    return { success: true };
  }
                          ...
                          <InlineStack align="space-between" blockAlign="center">
                            <InlineStack gap="300" blockAlign="center">
                              <BlockStack gap="200" >
                                ...
                                <Collapsible tone="subdued" open={openIndex === index} id={`collapsible-${index}`} transition={{ duration: '200ms', timingFunction: 'ease-in-out' }}>
                                  {task.description}
                                </Collapsible>
                              </BlockStack>
                            </InlineStack>
