...
import { BlockStack, Box, InlineStack, Text, Badge, Tooltip, Collapsible, Button } from "@shopify/polaris";
...
export type SetupTasksProps = {
  ownerTasks: OwnerTask[];
  completedTasks: string[];
  completedTaskCount: number;
  markTaskComplete: (taskId: string) => void;
  handleReviewRequest: () => void;
  navigate: (url: string) => void;
  shop: string;
  toggleTaskCompletion: (taskId: string) => void;
};

const IconSVG: React.FC<{ completed: boolean; onClick?: React.MouseEventHandler<SVGSVGElement> }> = ({ completed, onClick }) =>
  completed ? (
          ...
          <InlineStack gap="200">
            <BlockStack gap="200">
              ...
              <Text as="p" fontWeight="bold">
                <Badge>{`${completedTaskCount} / ${ownerTasks.length} Completed`}</Badge>
              </Text>
            </BlockStack>
          </InlineStack>
        </InlineStack>
      </div>
      <Collapsible open={true} id="ownerTasks">
        <BlockStack>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background:
                    hoveredIndex === index
                      ? "var(--p-color-bg-fill-transparent-hover)"
                      : openIndex === index
                        ? "var(--p-color-bg-fill-transparent-selected)"
                ...
                <InlineStack align="space-between" blockAlign="center">
                  <InlineStack gap="300" blockAlign="center">
                    <BlockStack gap="200" >
                      ...
                      <Collapsible open={openIndex === index} id={`collapsible-${index}`} transition={{ duration: '200ms', timingFunction: 'ease-in-out' }}>
                        {task.description}
                      </Collapsible>
                    </BlockStack>
                  </InlineStack>
